...
if request.method == 'POST':
    ...
    existe = supabase.table("folios_registrados").select("*").eq("folio", folio).execute().data
    if existe:
    ...
    supabase.table("folios_registrados").insert(datos).execute()
...
@app.route('/consulta_folio', methods=['GET', 'POST']) def consulta_folio(): resultado = None if request.method == 'POST': folio = request.form['folio']
